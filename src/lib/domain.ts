...
export interface StudyGoals extends GoalsInput { id: string; isActive: number }

export const DEFAULT_GOALS: Readonly<StudyGoals> = Object.freeze({
  id: "default-goal", targetListening: 7, targetReading: 7, targetWriting: 6.5,
  targetSpeaking: 6.5, targetOverall: 7, isActive: 1,
});
...
const bookBackupSchema = z.object({
  id: z.number().int().positive(), number: z.number().int(), title: z.string().min(1).max(500),
  version: testTypeSchema, isCustom: z.union([z.literal(0), z.literal(1)]).optional(),
}).transform((book) => ({ ...book, isCustom: book.isCustom ?? (book.number === 99 ? 1 : 0) }));
const practiceSetBackupSchema = z.object({
  id: z.string().min(1), bookId: z.number().int().positive().nullable(), testNumber: z.number().int().nullable(),
  moduleSkill: skillSchema, status: practiceSetStatusSchema, targetDate: z.string().nullable(),
  isCustom: z.union([z.literal(0), z.literal(1)]),
});
const attemptBackupSchema = z.object({
  id: z.string().min(1), practiceSetId: z.string().nullable(), date: z.string().min(1), skill: skillSchema,
  rawScore: z.number().int().min(0).max(40).nullable(), bandScore: bandScoreSchema,
  notes: z.string().nullable(), duration: z.number().int().positive().nullable(),
});
const goalsBackupSchema = goalsInputSchema.extend({ id: z.string().min(1), isActive: z.union([z.literal(0), z.literal(1)]) });
const settingBackupSchema = z.object({ key: z.string().min(1).max(200), value: z.string().max(10_000) });

export const backupPayloadSchema = z.object({
  version: z.literal(1),
  exportedAt: z.string().optional(),
  data: z.object({
    books: z.array(bookBackupSchema), sets: z.array(practiceSetBackupSchema),
    attempts: z.array(attemptBackupSchema), goals: z.array(goalsBackupSchema), settings: z.array(settingBackupSchema),
  }),
});
